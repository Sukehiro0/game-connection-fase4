...
// Ícones importados da biblioteca lucide-react para composição visual e gamificação
...
/**
 * @component Home
 * @description Landing Page principal da plataforma Game Connection. 
 * Implementa design responsivo com Tailwind CSS, feedback visual contínuo (animações fluidas),
 * e microinterações (Typewriter effect, Animated Counter).
 * @returns {JSX.Element} Estrutura da Home Page.
 */
export default function Home() {
  ...
  // ============================================================================
  // DADOS ESTÁTICOS E DICIONÁRIOS
  // ============================================================================

  /** 
   * @constant achievements 
   * @description Array de objetos contendo a iconografia e títulos para o grid de gamificação.
   */
  const achievements = [
  ...
  // ============================================================================
  // EFEITOS DE CICLO DE VIDA E ANIMAÇÕES MATEMÁTICAS
  // ============================================================================
  
  /**
   * @description Dispara as animações de carregamento inicial (Typewriter e Contador)
  ...
  useEffect(() => {
    ...
    const typeInterval = setInterval(() => {
      if (index <= fullSubtitle.length) {
        ...
        clearInterval(typeInterval); // Para a execução ao terminar a frase
      }
    }, 30); // Velocidade da digitação: 30ms por caractere

    // 2. EFEITO CONTADOR NUMÉRICO ANIMADO
    // Gera um número final aleatório entre 5.000 e 30.000 simulando base de dados
    ...
    const increment = finalValue / (duration / 20); // Calcula o salto de números a cada 20ms

    const counterInterval = setInterval(() => {
      startValue += increment;
      if (startValue >= finalValue) {
        setUserCount(finalValue);
        clearInterval(counterInterval); // Trava no número final exato
      } else {
        setUserCount(Math.floor(startValue)); // Atualiza o estado arredondando o valor
      }
    }, 20); // Taxa de atualização (aprox. 50 frames por segundo)

    // 3. CLEANUP FUNCTION (Prevenção de Memory Leak)
    // Se o usuário sair da página antes das animações terminarem, os intervalos são destruídos.
    return () => {
      clearInterval(typeInterval);
      clearInterval(counterInterval);
    };
  }, []); // Array de dependências vazio: executa apenas uma vez na montagem (mount)
    ...
    <main className="flex min-h-screen flex-col items-center px-4 relative overflow-hidden dark:bg-brand-dark transition-colors duration-500">
      ...
      <div className="container mx-auto max-w-6xl z-10 flex flex-col w-full">
        ...
        <div className="min-h-screen flex flex-col items-center justify-center text-center pt-20">
          ...
          {/* SUBTÍTULO ANIMADO (Typewriter rendering) */}
          ...
          {/* DISPLAY DE MÉTRICAS (Contador Social Proof) */}
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-16 py-20">
          ...
          <div className="flex-1 flex justify-center w-full" aria-label="Grid interativo de conquistas da plataforma" role="region">
            <div className="grid grid-cols-5 gap-3 md:gap-5">
              {achievements.map((achiev, index) => {
                ...
                // HARDCODE: Mantém o terceiro item (index 2) permanentemente ativo para destaque visual no layout
                ...
                return (
                  <div 
                    ...
                    className={`group/achiev w-14 h-14 md:w-20 md:h-20 rounded-xl flex items-center justify-center transition-all duration-300 cursor-pointer ${
                      isActive 
                        ? 'bg-brand-green/10 border border-brand-green scale-110 shadow-[0_0_25px_rgba(29,229,109,0.5)]'  // Estilo fixo ativo
                        : 'bg-[#05051a] border border-brand-green/20 hover:bg-brand-green/10 hover:border-brand-green hover:scale-110 hover:shadow-[0_0_25px_rgba(29,229,109,0.5)]' // Hover interativo
                    }`}
                    title={achiev.title}
                    role="img"
          ...
          {/* Lado Direito - Texto de Apoio e CTA Secundário */}
        ...
        {/* ================= SEÇÃO: NAVEGAÇÃO DE DESTAQUE (Cards Iniciais) ================= */}
